# source("/home/ben/research/NOC/SRS_wave_analysis/ERA5/analysis/ERA_trend_alldata_mpi_template.R")
...
# Test for at least 15% observations.
         if ( (sum(!is.na(as.vector(array_ERA[mat_lon_grid_idx[,lon_res_idx],mat_lat_grid_idx[,lat_res_idx],1,1]))) /
              length(as.vector(array_ERA[mat_lon_grid_idx[,lon_res_idx],mat_lat_grid_idx[,lat_res_idx],1,1]))) > 0.50 ) {
...
# Catch if lack of data for regression.
               if (all(!is.nan(df_Q[,(qq+1)]))) {
                  if (flag_reg == "NAO") {
                     ...
                     lm_Q <- lm(df_Q[,(qq+1)] ~ year + PDO_mean,data=df_Q)
                  } else if (flag_reg == "AO") {
                     lm_Q <- lm(df_Q[,(qq+1)] ~ year + AO_mean,data=df_Q)
                  } else {
