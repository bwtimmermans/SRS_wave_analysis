# source("/home/ben/research/NOC/SRS_wave_analysis/ERA5/analysis/ggplot_ERA_trend_NEA_template.R")
   library(ggplot2)
   library(grid)
   library(gridExtra)
...
# Chi plot.
   p1 <- ggplot(data = df_plot) + #scale_x_continuous(limits = c(-100, -50), expand = c(0,0)) + scale_y_continuous(limits = c(10, 50), expand = c(0,0)) +
         ylab("Latitude\n") + xlab("\nLongitude") +
         coord_map(xlim = c(260,370), ylim = c(20,70), projection = "lambert", lat0 = 10, lat1 = 40) +
...
# Map.
         geom_polygon(data = map_data("world"), aes(x = long, y = lat, group = group), color = "#000000", fill = NA, size = 0.55) +
# Multiple plots.
...
# Include blank panels.
   fig_file_name <- paste("./figures/ERA_trends/",res,"deg/trend_CI_",lab_stats[stat_idx],"_",paste(unlist(strsplit(lab_dataset,split=' ')),collapse='-'),"_",lab_years,"_",lab_months,"_",lab_y_centre,"_",flag_reg,"_NEA.png",sep="")
